...
list<string> translate(const map<string, string> & translator, const list<string> & sentence) noexcept {
    ...
    for (list<string>::const_iterator i=sentence.begin(); i!=sentence.end(); i++) {
        map<string,string>::const_iterator iter=translator.find(*i);

        if (iter!=translator.end()) { result.push_back(iter->second); } else { result.push_back("?"); }
...
bool areParenthesisPaired(const string & mathExpression) noexcept {
    ...
    for (size_t i=0; i<mathExpression.size(); i++) {
        ...
        if (c=='(' | c=='{' | c=='[' | c=='<') {
            ...
            if (par.empty()) return false;  // ak je prazdny, nemame s cim porovnavat, tak false
            // ak sa nezhoduju zatvorky, tak false
            if ((c==')' && par.top()!='(') || (c=='}' && par.top()!='{') || (c==']' && par.top()!='[') || (c=='>' && par.top()!='<')) return false;

            par.pop();
        }
    }
    return par.empty(); // ak prazdne vrati true
}
