...
double average(const initializer_list<int> & data) {
    ...
    return (std::accumulate(data.begin(), data.end(), 0.0))/data.size();    //ide po kazdom jednom a kazdy poscitava
}
...
list<int>::const_iterator findValue(const list<int> & data, int value) noexcept {
    return std::find(data.cbegin(), data.cend(), value); // konstantny zaciatok a kons koniec
}
...
list<int>::const_iterator findInRange200to400(const list<int> & data) noexcept {
    return std::find_if(data.cbegin(), data.cend(), isInRange200to400); // podmienka iba funkcia
}
...
list<int> add(const list<int> & data1, const list<int> & data2) noexcept {
    ...
    if (data1.size()==data2.size()) {
        ...
        // zaciatok prveho listu, koniec prveho listu, zaciatok druheho listu, kde sa bude ukladat vysledok, scitanie data1,2
